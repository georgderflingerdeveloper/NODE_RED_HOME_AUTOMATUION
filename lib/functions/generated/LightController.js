...
class LightController {
    ...
    process(payload = {}, nowMs = Date.now()) {
        ...
        if (payload.Command === 'AddOutput' || payload.Command === 'SetOutputCount') {
            this.cancelPendingClicks();
            const requestedCount = payload.Command === 'AddOutput'
                ? this.outputCount + 1 : payload.OutputCount;
    ...
    resizeOutputCount(value) {
        ...
        const normalScenarioId = this.normalScenarioIndex >= 0
        ...
        if (outputCount > previousCount) {
            let insertAt = definitions.findIndex(
                (scenario) => scenario.outputs.filter((level) => level > 0).length > 1,
            );
            if (insertAt < 0) insertAt = definitions.length;
            for (let index = previousCount; index < outputCount; index += 1) {
                const name = `LED ${index + 1}`;
                definitions.splice(insertAt, 0, {
    ...
    holdCurrentGroupScenario() {
        const scenario = this.getCurrentScenario();
        if (!scenario || scenario.outputs.filter((value) => value > 0).length < 2) return false;
        this.operatingMode = OPERATING_MODE_SCENARIO;
        this.modeOutputEnabled = true;
        this.manualLightIndex = null;
        this.normalScenarioIndex = this.scenarioIndex;
    ...
    applyAction(action, nowMs = Date.now()) {
        ...
        case 'next':
            ...
            this.nextScenario();
            return this.holdCurrentGroupScenario()
                ? 'scenario_group_selected' : 'scenario_changed';
        case 'previous':
            if (this.operatingMode !== OPERATING_MODE_NORMAL) return this.toggleSpecialModeOutput();
