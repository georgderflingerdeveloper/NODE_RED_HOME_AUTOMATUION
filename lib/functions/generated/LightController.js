...
function createScenarios(outputCount = DEFAULT_OUTPUT_COUNT) {
    ...
    // 1) Einzel-LED-Szenarien: [0], [1], [2], ..., [n-1]
    // 2) Gruppen-Szenarien: [0,1], [0,1,2], ..., [0,1,2,...,n-1]
    // Damit entsteht die systemseitige Grundfolge für die Lichtsteuerung.
...
// Erzeugt aus einem nutzerdefinierten Szenarionamen eine stabile, sichere ID.
// Unicode-Zeichen werden normalisiert, Sonderzeichen entfernt und bei
// Namenskonflikten wird automatisch ein eindeutiger Suffix angehängt.
...
// Erkennt die systemseitig erzeugten Standard-Szenarien anhand ihrer festen
// ID-/Namensmuster. So lassen sich Benutzer- und Basis-Szenarien sauber trennen.
...
// Erzeugt eine stabile Signatur für ein Szenario aus seinen Ausgangswerten.
// So können gleiche Lichtmuster zuverlässig erkannt und beim Migrieren von
// Konfigurationen korrekt abgeglichen werden.
function outputSignature(outputs) {
    return outputs.map((value) => Number(value).toFixed(6)).join('|');
}

// Die aktuelle Grundfolge hat stabile Rollen: zuerst jede LED einzeln, danach
// die wachsenden Gruppen. Die Werte eines solchen Szenarios dürfen vom Benutzer
// angepasst werden; seine Rolle in der Tasterfolge bleibt trotzdem erhalten.
function hasStructuredGeneratedLayout(scenarios, outputCount) {
    const expectedCount = (2 * outputCount) - 1;
    if (scenarios.length < expectedCount) return false;
    return scenarios.slice(0, expectedCount).every((scenario, index) =>
        scenario.id === `scenario-${index + 1}` && scenario.name === `Szenario ${index + 1}`);
}

// Die systemseitige Grundfolge folgt einem festen Muster: zuerst die
// Einzel-LEDs 1..n und danach die wachsenden Gruppen (2,3,4,...). Wenn die
// Anzahl der Ausgänge geändert wird, müssen die vorhandenen Rollen in der
// neuen Reihenfolge wiedergefunden werden. Dabei gilt: -1 = kein gültiger
// Index, null = jenseits der systemeigenen Grundfolge (also Benutzer-/Extraszenario).
// Die Berechnung hält die logische Bedeutung eines Szenarios erhalten, auch wenn
// die physische LED-Anzahl kleiner oder größer geworden ist.
function mapStructuredScenarioIndex(index, previousCount, outputCount) {
    if (!Number.isInteger(index) || index < 0) return -1;
    const previousSystemCount = (2 * previousCount) - 1;
    if (index >= previousSystemCount) return null;
    if (index < previousCount) return index < outputCount ? index : -1;
    const groupLength = index - previousCount + 2;
    return groupLength <= outputCount ? outputCount + groupLength - 2 : -1;
}

function rebuildGeneratedScenarioLayout(scenarios, outputCount) {
    // Bereits aktuelle Layouts bleiben unverändert. Das ist wichtig, weil ihre
    // Prozentwerte bewusst konfigurierbar sind und nicht auf 100/0 zurückgesetzt
    // werden dürfen.
    if (hasStructuredGeneratedLayout(scenarios, outputCount)) {
    ...
    const hasLegacyLedIdentity = generated.some((scenario) => /^led-\d+$/.test(String(scenario.id)));
    const isLegacyGeneratedLayout = hasLegacyLedIdentity && generated.length >= outputCount
        && generated.every((scenario) => canonicalSignatures.has(outputSignature(scenario.outputs)));
    if (!isLegacyGeneratedLayout) {
        return { definitions: scenarios, migratedIds: new Map() };
    }
    ...
    // Alte systemseitige Szenario-IDs werden anhand ihres Schaltmusters mit den
    // neuen, canonicalisierten Szenarien abgeglichen und in eine gültige Map
    // übersetzt, damit gespeicherte Zustände nach einer LED-Anzahl-Änderung
    // weiterhin korrekt aufgelöst werden.
    ...
    // Benutzerdefinierte Szenarien werden am Ende der neuen Standardliste
    // ergänzt; bei ID-Kollisionen werden sie mit einem eindeutigen Namen
    // neu vergeben, damit alle gespeicherten Referenzen sauber weiterlaufen.
...
class LightController {
    ...
    process(payload = {}, nowMs = Date.now()) {
        ...
        if (['SaveScenario', 'SelectScenario', 'ScenarioApply', 'ListScenarios', 'AddScenario', 'DeleteScenario', 'ClearAllScenarios']
    ...
    processScenarioCommand(payload, nowMs = Date.now()) {
        ...
        if ((command === 'SelectScenario' || command === 'ScenarioApply') && !rawName) {
            return this.getOutput({
        ...
        if (command === 'SelectScenario' || command === 'ScenarioApply') {
            if (existingIndex < 0) {
                return this.getOutput({
                    event: 'scenario_not_found',
            ...
            this.modeOutputEnabled = true;
            this.manualLightIndex = null;
            const explicitApply = command === 'ScenarioApply';
            return this.getOutput({
                event: explicitApply ? 'scenario_applied' : 'scenario_selected',
                commandResult: `${explicitApply ? 'Szenario angewandt' : 'Szenario aktiviert'}: ${this.scenarioDefinitions[existingIndex].name}`,
                persistentStateChanged: true,
            });
        }
    ...
    // Beim Ändern der LED-Anzahl bleibt die Rolle jedes Grundszenarios erhalten.
    // Benutzerwerte eines Grundszenarios werden in die neue Reihenfolge mitgenommen;
    // neue LED- und Gruppenszenarien entstehen mit den sicheren Standardwerten.
    resizeOutputCount(value) {
        ...
        const previousScenarioIndex = this.scenarioIndex;
        const previousNormalScenarioIndex = this.normalScenarioIndex;
        const activeScenarioId = this.getCurrentScenario()?.id ?? null;
        const normalScenarioId = previousNormalScenarioIndex >= 0
            ? this.scenarioDefinitions[previousNormalScenarioIndex]?.id ?? null : null;
        const structuredLayout = hasStructuredGeneratedLayout(
            this.scenarioDefinitions,
            previousCount,
        );
        ...
        if (structuredLayout) {
            ...
            // Bestehende konfigurierbare Grundszenarien werden anhand ihrer Rolle
            // (Einzel-LED bzw. Gruppengröße) in die neue Grundfolge verschoben.
            for (let index = 0; index < previousSystemCount; index += 1) {
                const targetIndex = mapStructuredScenarioIndex(index, previousCount, outputCount);
                if (targetIndex === null || targetIndex < 0) continue;
                definitions[targetIndex] = {
                    ...definitions[targetIndex],
                    outputs: [...resizedDefinitions[index].outputs],
                };
            }

            for (const scenario of resizedDefinitions.slice(previousSystemCount)) {
                const idCollision = definitions.some((item) => item.id === scenario.id);
            ...
            // Fremde oder bewusst völlig eigene Szenariolisten werden nicht
            // umsortiert. Neue Einzel-LEDs werden lediglich sicher ergänzt.
        ...
        if (structuredLayout) {
            const mappedActiveIndex = mapStructuredScenarioIndex(
                previousScenarioIndex, previousCount, outputCount,
            );
            const mappedNormalIndex = mapStructuredScenarioIndex(
                previousNormalScenarioIndex, previousCount, outputCount,
            );
            this.scenarioIndex = mappedActiveIndex === null
                ? definitions.findIndex((scenario) => scenario.id === activeScenarioId)
                : mappedActiveIndex;
            this.normalScenarioIndex = mappedNormalIndex === null
                ? definitions.findIndex((scenario) => scenario.id === normalScenarioId)
                : mappedNormalIndex;
        } else {
            this.scenarioIndex = activeScenarioId === null ? -1
                : definitions.findIndex((scenario) => scenario.id === activeScenarioId);
            this.normalScenarioIndex = normalScenarioId === null ? this.scenarioIndex
                : definitions.findIndex((scenario) => scenario.id === normalScenarioId);
        }
    ...
    // Für UI, Logs und Debugging wird der aktuelle Tastenzustand als
    // kompakte, lesbare Struktur exportiert, damit Drücken und Loslassen
    // sauber gemessen und dargestellt werden können.
