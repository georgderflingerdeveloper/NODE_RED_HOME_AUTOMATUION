...
<script type="text/x-red" data-template-name="retry-queue">
	<div class="form-row">
		<label for="node-input-name"><i class="icon-tag"></i> Name</label>
		<input type="text" id="node-input-name" placeholder="Name">
	</div>
	<div class="form-row">
		<label for="node-input-ack_mode">Reliability</label>
		<select id="node-input-ack_mode">
			<option value="ack">Ack</option>
			<option selected value="nack">Nack</option>
		</select>
	</div>
	<div class="form-row">
		<label for="node-input-timeout"><i class="fa fa-clock-o" aria-hidden="true"></i> Timeout (secs)</label>
...
<script type="text/x-red" data-help-name="retry-queue">
...
<script type="text/javascript">
	RED.nodes.registerType('retry-queue',
		{
			category: 'function',
			color: '#FFF0F0',
			defaults: {
				name: {value: ""},
				ack_mode: {value: "nack", required: true},
				timeout: {value: 30, required: true},
				sqlite: {required: true}
			},
			inputs: 1,
			outputs: 1,
			icon: "retry-queue-logo-small.png",
			label: function () {
