<script type="text/javascript">
    RED.nodes.registerType('phidget22-temperaturesensor',{
        ...
        label: function() {
            return "TemperatureSensor(" + this.channel + ")";
        },
        paletteLabel: "TemperatureSensor",
        oneditprepare: function() {
            var prevTCType = this.thermocoupleType;
            var prevRTDType = this.RTDType;
            ...
            $.getScript('phidget22/js/phidget22.min.js').done(function(data,textStatus,jqxhr){
                ...
                for (var key in phidget22.RTDType) {
                    $('#node-input-RTDType').append(new Option(key,phidget22.RTDType[key]));
                } 
                $('#node-input-RTDType').val(prevRTDType);

                for (var key in phidget22.ThermocoupleType) {
                    $('#node-input-thermocoupleType').append(new Option(key,phidget22.ThermocoupleType[key]));
                } 
...
<script type="text/x-red" data-help-name="phidget22-temperaturesensor">
    <p>Opens a Phidget temperature sensor object, which will send messages when the temperature changes.</p>
</script>